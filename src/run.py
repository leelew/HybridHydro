...
def main():
    ...
    default = dict(learning_rate=configs.learning_rate,
                   ...
                   id=configs.id,
                   att_type=configs.att_type,
                   gfs_inputs_shape=configs.gfs_inputs_shape)
    ...
    if configs.use_GFS:
        gfs_train = np.load(configs.ROOT + '/CLFS/inputs/GFS/GFS_0p1_f16_swvl1_interp_2015-2017.npy')
        gfs_id_train = gfs_train[:, :3, lat_id_low:lat_id_low + 112, lon_id_left:lon_id_left + 112]
        gfs_test = np.load(configs.ROOT + '/CLFS/inputs/GFS/GFS_0p1_f16_swvl1_interp_2018.npy')
    ...
    print(X_tf[0].shape, X_tf[1].shape)
    ...
    if wandb.config.model_name == 'unet':
        ...
        X = [X[0][:, :, :, :,-4:], X[1][:, :, :, :, -4:]]
    elif wandb.config.model_name == 'convlstm_tf':

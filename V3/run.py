...
def main(configs):
    ...
    for id in range(1, 25):
        ...
        path = configs.outputs_path+'forecast/'+configs.model_name+'_V3/'+str(id)+'/'
        if not os.path.exists(configs.outputs_path+'forecast/'+configs.model_name+'_V3/'):
            os.mkdir(configs.outputs_path+'forecast/'+configs.model_name+'_V3/')
